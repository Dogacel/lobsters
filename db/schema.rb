...
ActiveRecord::Schema[8.0].define(version: 2025_03_02_102016) do
  ...
  create_table "comments", id: { type: :bigint }, charset: "utf8mb4", collation: "utf8mb4_general_ci", force: :cascade do |t|
    ...
    t.index "to_tsvector('english'::regconfig, comment)", name: "index_comments_on_comment", using: :gin
  ...
  create_table "story_texts", charset: "utf8mb4", collation: "utf8mb4_general_ci", force: :cascade do |t|
    ...
    t.index "to_tsvector('english'::regconfig, (((((COALESCE(title, ''::character varying))::text || ' '::text) || COALESCE(description, ''::text)) || ' '::text) || COALESCE(body, ''::text)))", name: "index_story_texts_on_title_and_description_and_body", using: :gin
    t.index "to_tsvector('english'::regconfig, (COALESCE(title, ''::character varying))::text)", name: "index_story_texts_on_title", using: :gin
  end
  ...
  create_view "replying_comments", sql_definition: <<-SQL
    SELECT
        ...
        (
            ...
            WHERE v.user_id = read_ribbons.user_id
                AND v.comment_id = comments.id
        ) AS current_vote_vote,
        (
            SELECT v.reason FROM votes v
            WHERE v.user_id = read_ribbons.user_id
                AND v.comment_id = comments.id
        ) AS current_vote_reason
    FROM read_ribbons
    ...
    WHERE
        ...
        AND (
            ...
            OR (
                parent_comments.score > parent_comments.flags
                AND NOT parent_comments.is_moderated
                AND NOT parent_comments.is_deleted
            )
        )
        AND NOT EXISTS (
            SELECT 1 FROM votes f
        ...
        AND NOT EXISTS (
            SELECT 1 FROM hidden_stories h
